...
import com.Lchasi.train.gateway.util.JwtUtil;
...
public class LoginMemberFilter implements Ordered, GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        String path = exchange.getRequest().getURI().getPath();//获取请求地址
        ...
        if (validate) {
            ...
            exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);//返回码401，没有权限
            return exchange.getResponse().setComplete();
        }
