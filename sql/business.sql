...
# 两张表关联，如果有唯一键用唯一键管理，没有在考虑用ID管理
# 是否加冗余字段，看使用场景是查多还是写多，查多就可以加冗余
# 从表，用train_code关联
# 唯一键的作用是用来防止数据库出错，保存数据时，应该先查是否已存在，而不能认为有唯一键，不用查了，直接插入。唯一键是最后一道屏障，保证数据唯一性
...
# column是mysql关键字，用关键字作序号，在正常使用mybatis时是没问题，mybatis会自动增加反引号''，但在分布式事务seata时会有问题
